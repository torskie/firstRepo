...
/// <summary>
/// Summary description for EDIREPORT_corp
/// </summary>
public class EDIREPORT_corp
{
	public EDIREPORT_corp()
	{
		//
		// TODO: Add constructor logic here
		//
	}
    public Response insert(List<edireport_corp> data, String edi)
    {
        Response response = new Response();
        SqlTransaction transaction = null;
        string sql = "Insert into EDIREPORT_corp(bcode, descs,month_eli, year_eli, " +
                                "date_time, coalesce(sys_creator,''),rowguid) " +
                                "values " +
                                "(@bcode,@descs, @month_eli, @year_eli, " +
        ...
        try
        {
            using (SqlConnection connection = new DBConnection().getEDIConnection(edi))
            {
                using (SqlCommand command = new SqlCommand(sql, connection))
                {
                    ...
                    command.Parameters.Add("@descs", System.Data.SqlDbType.VarChar);
                    ...
                    try
                    {
                        ...
                        for (i = 0; i < data.Count; i++)
                        {
                            ...
                            command.Parameters["@descs"].Value = temp.descs;
